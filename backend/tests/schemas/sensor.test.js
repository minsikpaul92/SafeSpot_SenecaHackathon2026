...
describe('AlertLevelSchema', () => {
  const validAlert = { level: 'danger', message: 'Extreme Heat Warning - Find a Cool Space Now' };
  ...
  it('rejects invalid level value', () => {
    const result = AlertLevelSchema.safeParse({ level: 'critical', message: 'test' });
    expect(result.success).toBe(false);
  });
...
describe('SensorReadingSchema', () => {
  const validReading = {
    ...
    source: 'sensor',
    alert: { level: 'danger', message: 'Extreme Heat Warning - Find a Cool Space Now' },
  };
  ...
  it('accepts override source', () => {
    const result = SensorReadingSchema.safeParse({ ...validReading, source: 'override' });
    expect(result.success).toBe(true);
  });

  it('rejects invalid source', () => {
    const result = SensorReadingSchema.safeParse({ ...validReading, source: 'manual' });
    expect(result.success).toBe(false);
  });
...
describe('OverrideResponseSchema', () => {
  it('accepts valid override response', () => {
    const result = OverrideResponseSchema.safeParse({ status: 'overridden', temperature: 31.0 });
    expect(result.success).toBe(true);
  });

  it('rejects wrong status', () => {
    const result = OverrideResponseSchema.safeParse({ status: 'ok', temperature: 31.0 });
    expect(result.success).toBe(false);
  });
